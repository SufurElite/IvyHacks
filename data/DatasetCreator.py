...
def makeDatasets(board, evaluation, moveCnt, preferred=False):
	fpath = ""
	if preferred:
		fpath = "prefer"
	else:
	...
	if moveCnt < 40:	# Before the 40 plys we consider the moves that have been
		...
		MlpBitmaps(board,evaluation,STORING_PATH+fpath+'MlpFileOpening.txt')
		CnnBitmaps(board,evaluation,STORING_PATH+fpath+'CnnFileOpening.txt')
	else:
		...
		if cp <= 12:	# If the value of the pieces on the Board is smaller
			...
			MlpBitmaps(board,evaluation,STORING_PATH+fpath+'MlpFileEnd.txt')
			CnnBitmaps(board,evaluation,STORING_PATH+fpath+'CnnFileEnd.txt')

		elif cp > 12:	# Otherwise we are still in the MiddleGame
			
			MlpBitmaps(board,evaluation,STORING_PATH+fpath+'MlpFileMiddle.txt')
			CnnBitmaps(board,evaluation,STORING_PATH+fpath+'CnnFileMiddle.txt')
		else:
			pass
...
def process_game(game):
	...
	while not node.is_end():
		...
		try:
			...
			turn = GM_board.turn
			uci_move = next_node.move
			GM_move = str(node.board().san(uci_move))
			GM_board.push_san(GM_move)
			if turn==isWhite:
				res = engine.analyse(GM_board, chess.engine.Limit(time=movetime))
				...
				if info is not None:				
					...
					makeDatasets(GM_board, stock_evaluation, tmp, True)
				#print("NOT PREFERRED")
				#input(uci_move)
				
				for i in range(5):
					GM_board.pop()
					lg_moves = list(GM_board.legal_moves)
					...
					if mv!=GM_move:
						...
						if info is not None:				
							stock_evaluation = info/100 
							makeDatasets(GM_board, stock_evaluation, tmp, False)
				GM_board.pop()
				GM_board.push_san(GM_move)
			node = next_node
			tmp = tmp + 1
